...
hashT InicializirajHashTab(int velTab);
int HashFunkcija(char* drzava, int velTab);
int InsertToHashListAndTreeSorted(hashT H, char* drzava, char* gradovi);
Position FindListPosition(char* drzava, hashT H);
...
Position NadiPoImenuDrzave(char* drzava, hashT H);
int PrintGradovi(StabloPos root, int minBrStan);

int main() {
	char file[MAX] = { 0 }, Input[MAX] = { 0 }, drzava[MAX] = { 0 };
	int velTab = TAB_SIZE, minBrStanovnika = 0;
	hashT H = NULL;
	Position current = NULL;
	H = InicializirajHashTab(velTab);
	...
	do {
		printf("Upisite drzavu koju zelite pretraziti(Eng,njem,hrv):\n");
		InputString(drzava);
		current = NadiPoImenuDrzave(drzava, H);
		if (current != NULL) {
...
hashT InicializirajHashTab(int velTab)
{
	...
	H->tabSize = velTab;
	H->hashLista = (Position*)malloc(sizeof(Position) * H->tabSize);
	if (H->hashLista == NULL) {
		printf("Greska kod inicijalizacije memorije!");
		return NULL;
	}
...
int InsertToHashListAndTreeSorted(hashT H, char* drzava, char* gradoviFile)
{
	Position newEl = NULL;
	Position current = NULL;
	int hashKey = HashFunkcija(drzava, H->tabSize);
...
StabloPos Insert(StabloPos root, char* grad, int brojSt)
{
	...
	if (node == NULL) {
		StabloPos temp = NULL;
		temp = StvoriNewCvor(grad, brojSt);
		return temp;
	}
...
{
	Lista L = NULL;
	L = H->hashLista[HashFunkcija(drzava, H->tabSize)];
	while (L != NULL) {
